...
my $maf= shift or die "perl $0 maf\n";
my $spe= shift or die "ref_spe\n";
my $percent=0.1;
my $length_of_scaffold_plotted=50000;
...
foreach my $cattle_chr(sort {$a cmp $b} keys %link){
    foreach my $cattle_start(sort {$a<=>$b} keys %{$link{$cattle_chr}}){
		...
		#print "$cattle_chr\t$cattle_start\t$cattle_strand\n";
		#print "$wisent_chr\t$wisent_start\t$wisent_strand\n";
		#die;
...
foreach my $chr(sort {$a cmp $b} keys %result){
	foreach my $wisent_chr(sort {$result{$chr}{$a}<=>$result{$chr}{$b}} keys %{$result{$chr}}){
		my $pos=$result{$chr}{$wisent_chr};
		my $chr_len=$wisent_len{$wisent_chr};
		print Y"$chr\t$wisent_chr\n";
	}
}
close Y;
